...
def drawboard(board):
  #if growTimer <= 2000:
  #  g.DISPLAYSURF.blit(tileIMG_0, (x - (22*i), y + (6*i)))
  #elif growTimer > 2000 and growTimer <= 6000:
  #  g.DISPLAYSURF.blit(tileIMG_1, (x - (22*i), y + (6*i)))
  #elif growTimer > 6000 and growTimer <= 14000:
  #  g.DISPLAYSURF.blit(tileIMG_2, (x - (22*i), y + (6*i)))
  #elif growTimer > 14000:
  #  g.DISPLAYSURF.blit(tileIMG_3, (x - (22*i), y + (6*i)))
  activitylist = createactivitylist(board)
  activitylist = sortheightactivitylist(activitylist)
  for i in range (ACTIVELIST):
    g.DISPLAYSURF.blit(player_hold_bar, (0, 25))
    g.DISPLAYSURF.blit(player_hold_bar, (0, 425))
    g.DISPLAYSURF.blit(activitylist[i].image, (activitylist[i].ltposx, activitylist[i].ltposy))
        ...
        for piece in range (len(enemy.enemylist)):
          if enemy.enemylist[piece].istaken == 0: 
            if enemy.enemylist[piece].i == i and enemy.enemylist[piece].j == j and enemy.enemylist[piece].k == k:
              ...
              g.DISPLAYSURF.blit(enemy.enemylist[piece].image, (x, y))
        if player.istaken == 0:
          if player.i == i and player.j == j and player.k == k:
            x = board[i][j][k].ltposx + 14
            y = board[i][j][k].ltposy - player.height
            g.DISPLAYSURF.blit(player.image, (x, y))
  player.clearmovelist()

def drawhold(player):
  for i in range (len(player.holdlist)):
    if  player.holdlist[i] == 1:
      g.DISPLAYSURF.blit(enemy_pawn_IMG, (10 + 35*i, 20))
    elif player.holdlist[i] == 2:
      g.DISPLAYSURF.blit(enemy_queen_IMG, (10 + 35*i, 15))
    elif player.holdlist[i] == 10:
      g.DISPLAYSURF.blit(joker_IMG, (10 + 35*i, 20))

def drawhand(player):
  for i in range (len(player.handlist)):
    if  player.handlist[i].card_id == 1:
      g.DISPLAYSURF.blit(player.handlist[i].image, (player.handlist[i].ltposx, player.handlist[i].ltposy))
...
def drawtime(player, enemy):
  ...
  if player.istaken == 1:
    ...
    g.DISPLAYSURF.blit(textSurfaceObjPlayerTurn, textRectObjPlayerTurn)
    g.DISPLAYSURF.blit(textSurfaceObjPlayerTotal, textRectObjPlayerTotal)
    g.DISPLAYSURF.blit(textSurfaceObjEnemyTurn, textRectObjEnemyTurn)
    g.DISPLAYSURF.blit(textSurfaceObjEnemyTotal, textRectObjEnemyTotal)
